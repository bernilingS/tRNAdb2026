...
For easy retrieval and global searches, sequences and their most relevant data are stored within a single, fully indexed relation. This includes nucleotide sequence, secondary structure, anti codon, amino acid and origin (i.e., nuclear DNA, nuclear RNA, mitochondrial DNA). Every sequence has a taxonomic identifier and a separate table holds relevant taxonomic information which is obtained from the NCBI taxonomy database. To facilitate legacy compatibility, the original tRNA dataset \cite{juehling:2009} remains unaltered, except for an updated taxonomy assignment which is consistent with the current NCBI taxonomy database. We used the NCBI taxonomy data dump \TODO{VERSION,DATE} together with the \texttt{biopython} \texttt{entrez} package to query NCBI taxonomy API for updated taxonomic information for tRNAdb legacy sequences.

The updated relational schema supports a versioning system which separates the legacy dataset and data contained in tRNAdb 2.0 update. Every sequence present in tRNAdb has a version identifier attached which links it to relevant version information. For every version, data sources and software (with corresponding version) used are documented. Additionally, every tRNAdb version is described with a public version string that is exposed on the ``Details'' page of individual entries, and is also available as an export field. The public version string contains a short description how the sequences were obtained. \TODO{A more detailed description, e.g., software and datasources is available on the ``Documentation and Help'' page of tRNAdb.  Sequences in tRNAdb can be filtered by their corresponding version, allowing users to browse exclusively legacy data, or current releases.}
...
With this update we provide a new public HTTP interface, accessible by POST JSON requests at \texttt{http://tdb.bioinf.uni-leipzig.de/api}. The API implements \TODO{OpenAPI} specifications and a thourough documentation for users is available on the ``Documentation'' page of the tRNAdb website. Fundamentally, our API supports nearly identical search parameters as the tRNAdb website and allows filtering by any combination of origin, species, clade, NCBI taxonomy ID, amino acid, anticodon, sequence and structural motifs, reference (genome accession, external source, publication), and \TODO{version}. For many fields, multiple values can be supplied and entries matching at least one are returned.

Internally, POST requests send to the API are parsed, an appropriate SQL statement is executed, and matching entries are returned as JSON. No prior registration, and no personal information (e.g., email address, API key) has to be provided to access the tRNAdb API but user requests are limited to one request per second and at most 1000 entries returned per request to ensure fair use. Every API response, given the request was valid, contains a \texttt{total\_results} field that states the total number of sequences matching a request and all relevant results can be retrieved slice-wise via an offset parameter. \TODO{For requests that match a large number of records, a second API providing export to tsv, csv, json, or fasta is available at \texttt{http://tdb.bioinf.uni-leipzig.de/export}. For usage details see the documentation page of the tRNAdb website.}
...
Upon a successful search, the user is presented with a paginated list of fitting tRNA sequences with a snapshot of data available for every sequence: origin, organism, amino acid, anticodon and its structurally annotated nucleotide sequence (separated into acceptor stem, D-loop, D-stem, etc.). Multi-level sorting of entries matching a search request is possible, i.e., results can be sorted by each present field (e.g., ascending by organism name, descending by amino acid). Every result entry, or every entry matching the current search request, can be stored in a temporary selection which, via the prominent "Add Selection to Download", can be committed to persistent storage for later access or download. Sequences committed sequences are saved under a chosen collection name, or tag. These colletions can be viewed in a download manager and for every collection, the number of bookmarked entries is displayed. A limited preview for every collection is available upon clicking the corresponding collection in the download manager.
...
We implemented a searchable taxonomy browser which displays an expandable tree view of the NCBI taxonomic database. Every node in the taxonomy tree is annotated with its \TODO{official} NCBI name as well as its NCBI taxonomy ID. The taxonomy browser only displays clades and specie, for which there are tRNA sequences present in tRNAdb. For every entry in the taxonomy browser, the number of tRNA sequences separated by origin (nuclear DNA, nuclear RNA, mitochondrial DNA) is displayed. \TODO{Every node in the taxonomy browser contains a button which allows to search tRNAdb for sequences belonging to this clade/species.}}
