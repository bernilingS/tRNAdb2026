...
\abstract{%\TODO{Check the journal's online author guidelines for guidance on requirements for the abstract and other components of the submitted manuscript. Abstracts must be able to stand alone and so cannot contain citations to the paper's references, equations, etc.}
  ...
  been used extensively, accumulating more than $1\,000$ citations. Here,
  ...
  in data coverage, containing more than 400,000 new mitochondrial and
...
\PFS{(exclusively in Java and HTML/CSS), the new release aligns} tRNAdb
...
API key) \PFS{is required for} access the tRNAdb API but user requests are
...
large number of records, \PFS{and hence constitute} bulk retrieval of tRNA sequences, a second API endpoint provides export to tsv, csv, json, or fasta which is available at \texttt{https://tdb.bioinf.uni-leipzig.de/export}. Note that the unique tRNAdb identifier every entry is connected to is stable, hence over changing releases, existing identifiers will not change and remain relevant for retrieval purposes. For usage details see the documentation page of the tRNAdb website. \TODO{OpenAPI, /api/v1, stable-ID retrieval, canonical URLs}
...
\TODO{chemically modified RNA}, mitochondrial DNA) amino acid, anticodon,
...
obtained from NCBI Nucleotide
\url{https://www.ncbi.nlm.nih.gov/nucleotide/}, accessed October 2024. We
...
curation. If so, the sequence predicted by MITOS2 was deemed correct and
...
the NCBI annotation.  After filtering (see below), this resulted in 305\,905
...
Archaeal genomes were sourced from the Bacterial and Viral Bioinformatics Resource Center (BV-BRC) database
\url{https://www.bv-brc.org}, accessed October 2024. Predictions were ran
on good quality genomes, as assessed by CheckM2 ($\geq$85\% complete,
...
against tRNAinf-arch and tRNAinf-arch-SeC models.  After filtering,
