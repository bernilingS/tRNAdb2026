...
\author[5]{Joern P\"{u}tz}
\author[6]{Peter F. Stadler}
\author[7]{Stephan H. Bernhart\ORCID{0000-0002-5928-9449}}
...
\address[6]{\orgdiv{Department}, \orgname{Organization}, \orgaddress{\street{Street}, \postcode{Postcode}, \state{State}, \country{Country}}}
\address[6]{\orgdiv{Department}, \orgname{Organization}, \orgaddress{\street{Street}, \postcode{Postcode}, \state{State}, \country{Country}}}
\address[7]{\orgdiv{LeiCeM - Leipzig Center of Metabolism}, \orgname{Leipzig University}, \orgaddress{\street{Ritterstr. 26}, \postcode{04109}, \state{Saxony}, \country{Germany}}}
...
published (\cite{gtrnadb, trnadb-ce, modomics}. \PFS{When access to the
  ...
  \texttt{tRNAdb} on a solid technical foundadtion, we also extend its
  user-interface by integrating \texttt{infernal} \cite{infernal} as search
  tool and offering comprehensive support for mitochondrial tRNAs based on
  \texttt{mitos2} \cite{mitos2}.}
...
anticodon loop). To filter suspiciously large D-loops and T-loops, we took
the maximum length of the respective in tRNAdb 2009, added 1
(resulting in \todo{X and Y for D and T loops respectively}) and discarded a
total of \todo{X} tRNAs with longer loops.
