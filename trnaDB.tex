...
\newcommand{\todo}[1]{\scriptsize\color{red}#1}
...
published (\cite{gtrnadb, trnadb-ce, modomics}. \PFS{When access to the
  \texttt{tRNAdb} was interrupted due to technical isses in 2024/2025, the
  ...
  \texttt{tRNAdb} on a solid technical foundadtion, we also included an API for automatic data retrievance. Furthermore, we extended its content by including leveraging existing nuclear tRNAs from the gtRNA database as well as mitochondrial tRNAs we identified using \texttt{mitos2}\cite{mitos2} and additional specialized approaches.}
...
For all novel sequences, we used \texttt{infernal} to align sequences to
