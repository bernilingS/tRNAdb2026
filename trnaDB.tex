...
\todo{Since the publication of the first ``Compilation of tRNA sequences and sequences of tRNA genes'' in 2005 \cite{sprinzl:2005}} and its extension as the tRNAdb 2009\cite{juehling:2009}, a number of databases either dedicated to or containing tRNA sequences and genomic locations have been published (\cite{gtrnadb, trnadb-ce, modomics}. However, when it was discontinued due to techical reasons in 2024, the 2009 implementation was still widely used by the community. We therefore endevoured to not only put the tRNAdb on a solid techical foundation again, but also to extend its content by leveraging published tRNA sequences from later databases as well as by applying more modern search tools \cite{infernal} as well as more sophisticated models for these search tools\cite{mitos2}.  
...
We expanded the mitochondrial tRNAs included into tRNAdb. We downloaded a total of 16,630 mitochondrial sequences as well as their annotations from ?NCBI? genbank. We used Mitos2\cite{mitos2} without detection of proteins and rRNA and the refseq89m on each mitochondrion to detect the tRNAs. We subsequently compared our findings to the annotations from genbank, and kept any tRNA that matched exactly. For small location discrepancies, we checked whether the acceptor stem sturcture predicted by infernal looked reasonable and kept the Mitos2 predicted location if it did. For strand discrepancies, we kept Mitos2 predicted strands if no suitable anticodon for the predicted tRNA was in the annotated anticodon stem and loop. This resulted in  \tode{a number} of predicted tRNAs.

\subsection{Genomic tRNAs}\label{subsecgenomictRNA}
For genomic tRNAs, we downloaded sequences, locations and genome ids from the gtRNA database\cite{gtrnadb}.
\subsection{modified tRNAs}\label{subsecmodifiedtRNA}
Modified tRNA sequences as well as their genomic variants were downloaded from the MODOMIC\cite{modomics} database and directly from the pdb. We removed sequences from the pdb that only had a total of one modification at the first or last base. We decided to keep legacy one letter codes and also to not use chinese characters for our one letter codes, so we added new one letter codes for  modifications not already in tRNAdb 2009. 
\subsection{Alignemnt and quality control}
For all novel sequences, we used infernal to align sequences to the respective tRNA covariance models (eukaryotic, bacterial or archaeal models from tRNAscan for nuclear tRNAs, the respective Mitos2 models for mitochondrial tRNAs). When an anticodon was annotated, we compared the annotated one with the one implicitly found by infernal (as center of the anticodon loop). To filter suspiciously large D-loops and T-loops, we took the maximum length of the respective loop in tRNAdb 2009, added 1 (resulting in \todo{X and Y for D and T loops respectively})and discarded a total of  \todo{X} tRNAs with longer loops.     
